...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 * 剔除供应商角色和admin角色
	 */
	public List<Role> findAllRoleOne(){
		List<Role> roles = UserUtils.getRoleList();
		if(roles!=null && roles.size()>0){
			for (int i=0;i<roles.size();i++) {
				if(roles.get(i).getId().equals(Global.getConfig("supplier.role")) 
						|| roles.get(i).getId().equals(MinaGlobalConstants.ADMIN_ROLE)){
					roles.remove(i);
				}
			}
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 * @return
	 */
	public List<User> verify(User u) {
		return userDao.verify(u);
	}
