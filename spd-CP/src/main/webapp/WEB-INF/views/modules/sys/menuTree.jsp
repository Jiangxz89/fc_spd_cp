...
<body> --%>
<style>
	#left>div.accordion{background:#22A7FF;border-radius: 7px;padding-left: 20px;padding-bottom:10px;}
	#left>div.accordion>.accordion-group{margin:0;border:none;}
	#left>div.accordion>.accordion-group .accordion-heading{background:none;background-image:none;}
	#left>div.accordion>.accordion-group .accordion-heading>a.accordion-toggle{color:#fff;font-size:13px;padding:12px 15px 12px 0;}
	#left>div.accordion>.accordion-group>.accordion-body>.accordion-inner{padding:0;border:none;}
	.nav>li>a:hover, .nav>li>a:focus{background:none;}
	.nav-list>.active>a, .nav-list>.active>a:hover, .nav-list>.active>a:focus{background:#344048;}
</style>
	<div class="accordion" id="menu-${param.parentId}"><c:set var="menuList" value="${fns:getMenuList()}"/><c:set var="firstMenu" value="true"/><c:forEach items="${menuList}" var="menu" varStatus="idxStatus"><c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&&menu.isShow eq '1'}">
		<div class="accordion-group">
		    <div class="accordion-heading">
		    	<a class="accordion-toggle" data-toggle="collapse" data-parent="#menu-${param.parentId}" data-href="#collapse-${menu.id}" href="#collapse-${menu.id}" title="${menu.remarks}"><i style="float:right;" class="icon-chevron-${not empty firstMenu && firstMenu ? 'down' : 'right'}"></i>&nbsp;${menu.name}</a>
		    </div>
