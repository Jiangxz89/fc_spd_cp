	...
	<div class="right-main-box"> 
		<div class="searchBox">
			<form:form id="searchForm" modelAttribute="medstoYpcgzd" action="${ctx}/hys/medstoYpcgzd/list" method="post" class="breadcrumb form-search">
				...
				<div>
					<label>订单日期</label>
					<input type="text" name="queryDate" id="purchaseDate"  value="${medstoYpcgzd.queryDate }"/>
				</div>
				<div>
					<label>订单编号</label>
					<form:input path="djh" />
				...
				<tbody>
					<c:forEach items="${page.list }" var="drug">
						<tr>
							...
							<c:choose>
								<c:when test="${drug.jlzt == 1}">
									<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgzd/viewIn?xh=${drug.xh }&oprt=view">查看</a>
								</c:when>
								<c:when test="${drug.jlzt == 3}">
									<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgzd/viewIn?xh=${drug.xh }&oprt=view">查看</a>
								</c:when>
								<c:otherwise>
									<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgmx/form?cgxh=${drug.xh }&oprt=view">查看</a>
								</c:otherwise>
								</c:choose>
								<c:if test="${drug.jlzt == 0}">
									<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgmx/form?cgxh=${drug.xh }&oprt=audit">审核</a>
								</c:if>
						</td></shiro:hasPermission>
<%-- 							<td>
								<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgmx/form?cgxh=${drug.xh }&oprt=view">查看</a>
								<c:if test="${empty fns:getUser().supplierId and drug.jlzt eq 0}">
									<a class="hcy-btn hcy-btn-o" href="${ctx }/hys/medstoYpcgmx/form?cgxh=${drug.xh }&oprt=audit">审核</a>
								</c:if>
							</td> --%>
						</tr>
